...
@Component({
  selector: 'app-transferred-product-based',
  templateUrl: './transferred-product-based.component.html',
  styleUrls: [ './transferred-product-based.component.scss' ]
})

export class TransferredProductBasedComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.transfer_id = Number(this.activatedRoute.snapshot.paramMap.get('transfer_id'))
    ...
    if (this.transfer_id) {
      this.transferBetweenWarehouses.controls[ 'transfer_number' ].setValue(this.transfer_id)
    }
  ...
  getAllData(filters: any, paginated: boolean) {
    ...
    let getUrl = ''
    if (paginated) {
      getUrl = 'warehouses/transfers/products';

    }
    else {
      getUrl = 'warehouses/transfers/products/all';
    }
      ...
      tempArr.push(
        {
          ...
          'product_location': transfer.batch.main_location?.number + '/' + transfer.batch.main_location?.stand + '-' + transfer.batch.main_location?.shelf,
          'id': transfer.transfer.id,
          'name': transfer.batch.product.name,
          'exp_operating': transfer.batch.expired_at + ' ' + transfer.batch.operating_number,
  ...
  navigateToDetails(id: any) {
    this.router.navigate([ `/warehouse/storekeeper/warehouse-transfers/transferred-product-based/${id}` ])
  }

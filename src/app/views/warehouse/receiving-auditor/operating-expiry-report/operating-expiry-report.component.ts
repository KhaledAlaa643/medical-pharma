...
@Component({
    selector: 'app-operating-expiry-report',
    templateUrl: './operating-expiry-report.component.html',
    styleUrls: [ './operating-expiry-report.component.scss' ]
})

export class OperatingExpiryReportComponent implements OnInit, OnDestroy {
    ...
    getDropdownData() {
        this.subs.add(this.generalService.getDropdownData(['manufacturers','suppliers']).subscribe({
            next: res => {
    ...
    ngOnInit(): void {
        if(localStorage.getItem('multiple_corridors_enabled')){
            ...
            if(this.multiple_corridors_enabled=='false'){
              const location_index_header= this.columnsArray.findIndex((c:any)=>c.field=='المحطة')
              const location_index_value= this.columnsName.findIndex((c:any)=>c.field=='location')
                    ...
                    this.operatingExpiryReportData.push(
                        {
                            ...
                            supplier_name: element?.supplier !=null ? element?.supplier?.name: 'مرتجع',
                            location: element?.corridor?.number + " / " + element?.shelf + " " + element?.stand,
                            created_by: element?.created_by?.name,
                            supplied_at: element?.supplied_at!=null ? element.supplied_at.split(" ")[ 0 ] : element.created_at.split(" ")[ 0 ] ,
    ...
    getOperatingExpiryReportFilter(filter: any) {
        ...
        if (x[ 'sort_by' ] == null || x[ 'sort_by' ] == '' || x[ 'sort_by' ] == undefined) {
            x[ 'sort_by' ] = 'expired_at'
        }
        if (x[ 'direction' ] == null || x[ 'direction' ] == '' || x[ 'direction' ] == undefined) {
            x[ 'direction' ] = 'asc'
        }
    ...
    getUpdatedQueryParams(sorting_data?: any) {
        ...
        if (this.operatingExpiryReportFilter.controls[ 'supplied_at' ].value) {
            queryParams[ 'supplied_at' ] = datePipe.transform(new Date(this.operatingExpiryReportFilter.controls[ 'supplied_at' ].value), 'yyyy-MM-dd');
        }
        return queryParams;
    }

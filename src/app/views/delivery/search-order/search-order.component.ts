...
export class SearchOrderComponent implements OnInit {
  ...
  getClientidFromCode(param: string) {
    ...
    } else if (param == 'clientId' && this.clientFilter.controls[ 'pharmacy_id' ].value) {
      this.loaderBooleanId = true

      this.clientFilter.controls[ 'pharmacy_id' ].setValue(this.clientFilter.controls[ 'pharmacy_id' ].value)
      ...
      this.subs.add(this.generalService.getPharmacies(params).subscribe({
        next: res => {
          ...
          if (param == 'code' && this.clientFilter.controls[ 'code' ].value) {
            ...
            this.clientFilter.controls[ 'pharmacy_id' ].setValue(res.data[ 0 ]?.id)
          }
          if (this.client?.code) this.clientFilter.controls[ 'code' ].setValue(String(this.client?.code))
        ...
        next: res => {
          ...
          res.data.forEach((element: any) => {
            ...
            this.ClientInvoices.push({
              ...
              'client_name': element?.client?.name + ' ' + element?.pharmacy?.name,
  ...
  viewReturnDetails(ids: any) {
    this.router.navigate([ `/delivery/register-return-permission/${ids.id}/${ids.pharmacy_id}` ])
  }

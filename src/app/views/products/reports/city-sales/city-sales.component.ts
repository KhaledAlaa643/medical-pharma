...
@Component({
  selector: 'app-city-sales',
  templateUrl: './city-sales.component.html',
  styleUrls: ['./city-sales.component.scss']
})
export class CitySalesComponent implements OnInit {
 private subs = new Subscription();
  ...
  rows!: number
  total: number=0
  area_pharmacies_count_sum:number=0
...
changepage(event: any) {
  ...
  getReportData() {
    if (this.filterForm.valid) {
      ...
      this.subs.add(this.http.getReq('products/reports/city-sales', { params: queryParams }).subscribe({
        next: res => {
           this.tableData=res.data
           this.tableFooter=[{...res.additional_data,total_text:'الاجمالي'}]
  ...
  getCities() {
    this.subs.add(this.generalService.getArea().subscribe({
      next: (res) => {
        this.areas = res.data
        this.area_pharmacies_count_sum = res.additional_data.pharmacies_sum
      }
    }))
  }

...
export class SupplyRequestComponent implements OnInit {
  ...
  // for first table (last orderd quantity)
  ...
  lastOrderedQuantity: any = [];

  // for second table (last discount)
  ...
  calcTotal(index?: any) {
    ...
    this.cartData.forEach((cart_product: any) => {
      ...
      this.supplyRequestTotals.total_supply_price =
        this.supplyRequestTotals.total_supply_price + cart_product.total;
    });
  ...
  addProductToCart() {
    ...
    if (
      ...
      body['pharmacy_id'] = this.requestDataForm.controls['pharmacy_id'].value;
      body['is_supplier'] = 0;
    }
    let cartProduct: any = {};
    let message: string = '';
  ...
  deleteQuantity() {
    ...
    if (this.purchase_id) {
      this.DeleteWithQuantity.controls['purchase_id'].setValue(
        this.purchase_id
      );
    }
    this.subscription.add(
      this.http
        .deleteReq('purchases/supply-request/delete-items-by-quantity', {
          params: this.DeleteWithQuantity.value,
        })
        .subscribe({
          next: (res) => {
  ...
  @ViewChild('suppliersDropdown') private suppliersDropdown!: Dropdown;
  @ViewChild('warehousesDropdown') private warehousesDropdown!: Dropdown;
  @ViewChild('productsDropdown') private productDropdown!: Dropdown;

  focusSupplier(dropdown: any) {
    if (!dropdown.overlayVisible) {
      this.suppliersDropdown.focus();
      dropdown?.close();
    }
  }
  focusWarehouse(dropdown: any) {
    if (!dropdown.overlayVisible) {
      this.warehousesDropdown.focus();
      dropdown?.close();
    }
  }
  focusProduct(dropdown: any) {
    if (!dropdown.overlayVisible) {
  ...
  deleteSupplyRequest() {
    let message: string = '';
    let all_params = {
      purchase_id: this.addProductForm.controls['purchase_id'].value,
    };
    this.subscription.add(
      this.http
  ...
  saveSupplyRequest(status: number) {
    ...
    if (this.order_note != undefined && this.order_note != null) {
      formDataPayLoad.set('order_note', String(this.order_note));
    }

    let url = '';
    if (this.editOrder) {
      url = 'purchases/supply-request/update-request';
    } else {
      url = 'purchases/supply-request/submit-request';
    }
  ...
  filter() {
    ...
    if (this.type === 'supplier') {
      delete all_params['pharmacy_id'];

      all_params['supplier_id'] =
        this.requestDataForm.controls['supplier_id'].value;
      all_params['is_supplier'] = 1;
    } else if (this.type === 'client') {
      delete all_params['supplier_id'];
      all_params['pharmacy_id'] =
        this.requestDataForm.controls['pharmacy_id'].value;
      all_params['is_supplier'] = 0;
    }
      ...
      this.http
        ...
        .subscribe({
          next: (res) => {
            ...
            this.lastOrderedQuantity = [];
            this.lastDiscount = [];
            if (res.data.last_notInvoiced) {
              this.lastOrderedQuantity.push({
                order_number: res.data.last_notInvoiced?.id,
                supplier_name: res.data.last_notInvoiced.supplier.name,
                created_at: res.data.last_notInvoiced.created_at,
                quantity: res.data.last_notInvoiced.quantity,
                discount: res.data.last_notInvoiced.discount,
                created_by: res.data.last_notInvoiced.created_by.name,
              });
            }
            if (res.data.last_invoice) {
              this.statisticsForm.controls['last_discount'].setValue(
                res.data.last_invoice?.discount
              );
              this.lastDiscount.push({
                invoice_number: res.data.last_invoice?.id,
                supplier_name: res.data.last_invoice?.supplier.name,
                created_at: res.data.last_invoice?.created_at,
                quantity: res.data.last_invoice?.quantity,
                discount: res.data.last_invoice?.discount,
                price: res.data.public_price,
                created_by: res.data.last_invoice?.created_by.name,
              });
            }
          },
        })

...
@Component({
  selector: 'app-logbook-receiving-record',
  templateUrl: './logbook-receiving-record.component.html',
  styleUrls: ['./logbook-receiving-record.component.scss'],
})
export class LogbookReceivingRecordComponent {
  ...
  getAllData(filters: any, paginated: boolean) {
    ...
    let getUrl = '';
    if (paginated) {
      getUrl = 'accounting/log-books/received';
    } else {
      getUrl = 'accounting/log-books/received/all';
    }
  ...
  // reviewLogbookOne(event: any) {
  //   this.openReviewModal();
  //   this.reviewForm.patchValue(event);
  // }

  confirmReviewOneLogbook(event: any) {
    let message = '';
    let index = this.data.findIndex((item: any) => item.id == event.id);
    this.data[index].is_reviewed = true;
    this.subscription.add(
      this.http
        .putReq(
          `accounting/log-books/archive-one/${event.id}`
          // {
          //   remaining_receipts: this.reviewForm.value.remaining_receipts,
        ...
        .subscribe({
          ...
          complete: () => {
            ...
            if (!this.reviewedIds.includes(event.id)) {
              this.reviewedIds.push(event.id);
            }
          },
          error: (err) => {
            let index = this.data.findIndex((item: any) => item.id == event.id);
            this.data[index].is_reviewed = false;
          },
        })
    );
  }

  confirmReviewManyLogbook() {
    let message = '';

    this.subscription.add(
      this.http
        .putReq(`accounting/log-books/archive-many`, {
  ...
  @ViewChild('reviewOpen') reviewOpen!: ElementRef<HTMLElement>;

  openReviewModal() {
    let el: HTMLElement = this.reviewOpen.nativeElement;
    el.click();
  }

  // @ViewChild('reviewManyOpen') reviewManyOpen!: ElementRef<HTMLElement>;

  openReviewManyModal() {
    console.log(this.data);
    this.logBooksId = [];
    this.data.forEach((item: any) => {
      if (!this.reviewedIds.includes(item.id)) {

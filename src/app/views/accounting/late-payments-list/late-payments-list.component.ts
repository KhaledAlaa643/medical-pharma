...
@Component({
  selector: 'app-late-payments-list',
  templateUrl: './late-payments-list.component.html',
  styleUrls: ['./late-payments-list.component.scss'],
})
export class LatePaymentsListComponent {
  ...
  getPharmacies() {
    console.log(this.searchForm.value.client_id);
    if (!this.searchForm.value.client_id) {
      this.pharmacies = [];
      return;
    } else {
      this.subscription.add(
        ...
        .subscribe({
          next: (res) => {
            ...
            this.data = res.data.map((data: any, index: number) => {
              ...
              data['payment_period'] = data.payment_period.name;
              return data;
            });
  ...
  getAllData(filters: any, paginated: boolean) {
    ...
    let getUrl = '';
    if (paginated) {
      getUrl = 'accounting/collections/late';
    } else {
      getUrl = 'accounting/collections/late/all';
    }
